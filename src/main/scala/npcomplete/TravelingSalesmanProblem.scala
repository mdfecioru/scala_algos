...
object TravelingSalesmanProblem extends App {

  val INFINITY = Int.MaxValue
  val adjMatrix = new ArrayBuffer[ArrayBuffer[Int]]()

  val filename = "src/main/resources/tsp_small.txt"
  val iter = Source.fromFile(filename).getLines()
  val nrVertex =iter.next().toInt
  val solution = iter.next().toInt
  ...
  println("minV = ", minV)
  if (minV == solution) println("Test passed.")
